...
import com.fup.jennyferlopez.proyectokitetkiwe.fragments.niveldos.ConsonantesActivity;
...
public class Nivel3Activity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        if (v.getId()== R.id.correAvatar){
            Intent ircolores= new Intent(getApplication(), ColoresActivity.class);
    ...
    private void actualizarActivity() {
        ...
        User usuario_por_id = servicioUsuario.obtenerUsuarioPorId(userName);
        if (usuario_por_id!=null) {
            servicioUsuario.actualizaractivity(usuario_por_id,"Nivel3Activity");
            int p=Integer.parseInt(String.valueOf(usuario_por_id.getPuntos()));
            tv_puntos.setText(""+ p);
        }
    }
